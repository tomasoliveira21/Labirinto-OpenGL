...
#define LARGURA_PAREDE 5
#define COMPRIMENTO_PAREDE 5
...
void desenhaCubo(GLfloat largura, GLfloat comprimento, GLfloat altura)
{

  GLfloat l = altura / 2;
  GLfloat c = comprimento / 2;
  GLfloat a = largura / 2;

  GLfloat vertices[][3] = {{-c, 0, -a},
                           {c, 0, -a},
                           {c, l, -a},
                           {-c, l, -a},
                           {-c, 0, a},
...
void setNavigateSubwindowCamera(Camera *cam, Objeto obj)
{
  ...
  cam->eye.x = obj.pos.x - 1 * cosf(RAD(GRAUS(-modelo.objeto.dir)));
  cam->eye.y = obj.pos.y + 1;
  cam->eye.z = obj.pos.z - 1 * sinf(RAD(GRAUS(-modelo.objeto.dir)));
  center.x = obj.pos.x;
...
void timer(int value)
{
  ...
  if (estado.teclas.up)
  {
    nx = modelo.objeto.pos.x + (1 * velocidade * cosf(RAD(GRAUS(modelo.objeto.dir))));
    nz = modelo.objeto.pos.z + (1 * velocidade * sinf(RAD(GRAUS(modelo.objeto.dir))));
    // calcula nova posição nx,nz
    if (!detectaColisao(nx, nz))
    {
      modelo.objeto.pos.x = nx;
      modelo.objeto.pos.z = nz;
      // modelo.tanque.x += (t.translacao * t.velocidade * cosf((t.direccao * M_PI) / 180));
      // modelo.tanque.y += (t.translacao * t.velocidade * sinf((t.direccao * M_PI) / 180));
    }
    andar = GL_TRUE;
  }
  ...
  {
    nx = modelo.objeto.pos.x - 0.7 * velocidade * cosf(RAD(GRAUS(modelo.objeto.dir)));
    nz = modelo.objeto.pos.z - 0.7 * velocidade * sinf(RAD(GRAUS(modelo.objeto.dir)));
    // calcula nova posição nx,nz
    if (!detectaColisao(nx, nz))
    {
      modelo.objeto.pos.x = nx;
  ...
  // printf("x = %f \t y = %f \t z= %f \t dir = %f \t vel= %f prev = %d\n", modelo.objeto.pos.x, modelo.objeto.pos.y, modelo.objeto.pos.z, GRAUS(modelo.objeto.dir), velocidade, modelo.prev);
  // printf("cos(%f)=%f\tsin(%f)=%f\n", GRAUS(modelo.objeto.dir), cosf(RAD(GRAUS(modelo.objeto.dir))), modelo.objeto.dir, sinf(RAD(GRAUS(modelo.objeto.dir))));
  redisplayAll();
}
